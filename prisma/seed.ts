...
async function main() {
  ...
  for (const studentData of studentProfiles) {
    ...
    const assessments = [
      { name: 'IA1', max_score: 50, score_obtained: Math.floor(Math.random() * 20) + 30 },
      { name: 'IA2', max_score: 50, score_obtained: Math.floor(Math.random() * 20) + 25 },
      { name: 'MidSem', max_score: 100, score_obtained: Math.floor(Math.random() * 30) + 40 },